...
```txt
# required:
DISCORD_TOKEN= # bot token. do not fucking share this shit with anyone who has no business having it
CLIENT_ID= # bot id
...
```
2. Register your bot's command: (Note: rerun this every time you modify the command's property (name, description, options), or just make it run every time the bot starts)
```bash
npm run deploy
```
